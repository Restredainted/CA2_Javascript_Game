...
class Camera {
	...
	update() {
		...
		/* Removed x tracking to keep the camera in a vertical position. No ground will be 
		generated outside the horizontal bounds of the camera, 
		thus there's no need for the camera to move side to side.*/
		...
		this.y = clamp((this.target.y + this.target.getComponent(Renderer).height / 2 - this.height / 2), 0, (this.target.game.canvas.height * 3.485 - this.target.game.canvas.height % 32));
	}
}
